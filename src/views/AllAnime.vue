  ...
  <div class="bg-[#0e0e0e] min-h-screen text-gray-200 font-sans">
    ...
    <header class="bg-[#1c1c1c] shadow-md text-white fixed w-full z-50 border-b border-[#00aaff]">
      <main class="max-w-6xl mx-auto flex justify-between items-center px-4 py-3">
        ...
        <!-- MENU -->
        <nav class="hidden md:flex space-x-6 items-center text-sm font-semibold">
          <a href="/" class="hover:text-[#00aaff]  text-white rounded-sm">HOME</a>
          <a href="/allanime" class=" px-3 py-1 rounded-sm bg-[#00aaff]">ANIME LIST</a>
      ...
      <transition name="fade">
        <nav v-if="menuOpen" class="bg-[#111] border-t border-[#00aaff] text-sm flex flex-col space-y-3 p-4 md:hidden">
          <a href="/">HOME</a>
          <a href="/allanime" class="text-[#00aaff]">ANIME LIST</a>
    ...
    <main class="max-w-6xl mx-auto pt-28 px-3">
      ...
      <section v-if="searchResults.length > 0" class="mt-6 bg-[#1a1a1a] p-4 rounded-md shadow">
        <div class="flex justify-between items-center">
          ...
          <button @click="clearSearch" class="text-[#00aaff] text-sm hover:underline">
            ✖ Bersihkan
          </button>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-3 mt-4">
          <div
...
async function searchAnime() {
  ...
  searchResults.value = []
  try {
    const res = await axios.get(`https://www.sankavollerei.com/anime/stream/search/${encodeURIComponent(search.value)}`)
    searchResults.value = res.data.data || []
  } catch (e) {
    console.error(e)
    errorMsg.value = "Anime tidak ditemukan."
  } finally {
    loading.value = false
  }
}
